...
std::expected<CarMetadata, std::string>  DBManager::queryCars(std::string jobname, int carid){
    ...
    std::string querystr = std::format("SELECT follow_a, follow_b, follow_c, politeness, desired_vel FROM CarData INNER JOIN TrafficJobs ON TrafficJobs.JobID = CarData.JobID WHERE jobname = '{}' and carid = {}", jobname, carid);
    ...
    try {
        ...
        return CarMetadata{modelParams, r["politeness"].as<float>(), r["desired_vel"].as<float>(), carid};
    } catch(const std::exception& e) {
...
std::expected<std::vector<CarMetadata>, std::string> DBManager::queryCars(std::string jobname){
    ...
    if (!connect){return std::unexpected(connect.error());}
    pqxx::work tx{*connect};
    std::string querystr = std::format("SELECT carid, follow_a, follow_b, follow_c, politeness, desired_vel FROM CarData INNER JOIN TrafficJobs ON TrafficJobs.JobID = CarData.JobID WHERE jobname = '{}'", jobname);

    std::vector<CarMetadata> data;
    
    try {
        for (auto [carid, a, b, c, p, v] : tx.query<int, float, float, float, float, float>(querystr)){
            data.push_back({{a,b,c}, p, v, carid});
        }
    }
    catch(const std::exception& e){
...
std::expected<std::vector<RawData>, std::string> DBManager::queryData(std::string jobname){
    ...
    alldata.back().id_ = 0;
    try {
        for (auto [id, x, v, t, l] : tx.query<int, float, float, float, float>(querystr)){
...
std::expected<TimeSeries, std::string> DBManager::queryData(std::string jobname, 
    std::optional<double>x0, std::optional<double>x1,
    std::optional<double>t0, std::optional<double>t1){

    // Set boundaries
    float xmin = x0.value_or(0.0);
    float xmax = x1.value_or(std::numeric_limits<float>::max());
    float tmin = t0.value_or(0.0);
    float tmax = t1.value_or(std::numeric_limits<float>::max());
    ...
    try {
        ...
        return std::unexpected(std::format("Error reading spatial data from job {}: {}", jobname, e.what()));
    }
    return data;
}
