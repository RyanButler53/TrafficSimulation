...
std::expected<void, std::string> Database::initDB(bool useTestDB){
    ...
    try {
        ...
        tx.exec("CREATE TABLE IF NOT EXISTS laneSegments (jobID int, segmentStart float, segmentEnd float, rate float, FOREIGN KEY (jobID) REFERENCES trafficJobs(jobID))");
