...
std::expected<void, std::string> initDB::initDB(bool useTestDB){
    ...
    try {
        ...
        tx.exec("CREATE TABLE IF NOT EXISTS trafficJobs ( jobID int GENERATED ALWAYS AS IDENTITY PRIMARY KEY, configfile text, jobname text, status varchar(7), error text, followModel varchar(5), numCars int, runtime float, x0 float, xf float, numLanes int)");
        ...
        tx.exec("CREATE TABLE IF NOT EXISTS laneSegments (jobID int, start float, end float, rate float), FOREIGN KEY (jobID) REFERENCES trafficJobs(jobId)");
        tx.commit();
        return {};
    }
